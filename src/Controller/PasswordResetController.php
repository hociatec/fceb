...
class PasswordResetController extends AbstractController
{
    ...
    public function requestReset(
        ...
        $data = new ForgotPasswordRequestData();
        $form = $this->createForm(ForgotPasswordRequestFormType::class, $data);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $user = $userRepository->findOneBy(['email' => mb_strtolower((string) $data->email)]);
    ...
    public function resetPassword(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $user
                ->setPassword($passwordHasher->hashPassword($user, (string) $data->plainPassword))
